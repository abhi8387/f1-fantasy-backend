...
Node.js, Express, PostgreSQL + Prisma, Redis, JWT auth (access + refresh tokens), Zod validation, Winston logging, Docker Compose, OpenF1 API.
...
## What's actually implemented (and verified against a live Postgres/Redis/OpenF1)
...
1. Copy `.env.example` to `.env` and fill in real secrets. Note the Postgres port is `5433`, not the default `5432` — this avoids clashing with a native Postgres install that might already be using `5432`.
2. Start infrastructure: `docker compose up -d postgres redis`
3. Install dependencies: `npm install`
4. Generate the Prisma client and run migrations: `npx prisma generate && npx prisma migrate dev`
